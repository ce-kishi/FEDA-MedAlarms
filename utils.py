...
def split_in_seqs(data, subdivs):  # input (X_train, seq_len)
    if len(data.shape) == 1:
        if data.shape[0] % subdivs:
            data = data[:-(data.shape[0] % subdivs), :]
        data = data.reshape((data.shape[0] // subdivs, subdivs, 1))
    elif len(data.shape) == 2:  # X,Yなど
        # 総フレーム数がseq_len(シーケンスの長さ)で割り切れないデータを除去
        if data.shape[0] % subdivs:  # 総フレーム数/seq_lenの余りがあれば
            data = data[:-(data.shape[0] % subdivs), :]
        # numpy.reshapeで、
        ...
        if data.shape[0] % subdivs:
            data = data[:-(data.shape[0] % subdivs), :, :]
        data = data.reshape(
            (data.shape[0] // subdivs, subdivs, data.shape[1], data.shape[2]))
    return data
